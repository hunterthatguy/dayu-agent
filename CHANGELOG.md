...
- Web UI Upload 分阶段触发流程（Ver 1.0.6）：
  - 四阶段流程：解析 ticker → 下载财报 → 解析与抽取 → 维度分析
  - 用户点击已完成的阶段块可手动触发下一阶段
  - 可点击阶段显示 amber 边框和「点击继续」提示
  - 维度分析展示财务指标摘要和关键洞察
- Web UI 财报文件预览增强（Ver 1.0.6）：
  - Filing 详情页原文标签页支持 PDF/HTML iframe 预览
  - 文件端点支持 `preview=true` 参数 inline 显示
  - 预览高度增加到 600px，并提供下载链接
- 后端 API 端点（Ver 1.0.6）：
  - `/api/upload/process`：触发解析与抽取阶段
  - `/api/upload/analyze`：触发维度分析阶段
- 后端服务（Ver 1.0.6）：
  - `DimensionAnalysisService`：财务指标分析服务
  - 支持营收、净利润、资产负债、现金流、EPS 等关键指标提取
  - 自动生成分析摘要和洞察
- SSE 事件流优化（Ver 1.0.6）：
  - 前端直接连接后端端口 9000，绕过 Vite 代理缓冲
  - 默认 CORS 配置允许前端端口 5175
  - 事件总线添加详细调试日志
